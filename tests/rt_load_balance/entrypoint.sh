#!/bin/bash
# Entrypoint for godiam rt_load_balance test
...
echo -e "${BLUE}==============================================${NC}"
echo -e "${BLUE}  godiam rt_load_balance Test${NC}"
echo -e "${BLUE}  Tests load balancing across multiple peers${NC}"
echo -e "${BLUE}==============================================${NC}"
echo ""

echo -e "${BLUE}[1/4] Starting godiam gw (with rt_load_balance)...${NC}"
/usr/local/bin/diameterd --config /conf/gw.yaml > /tmp/gw.log 2>&1 &
export PID_GW=$!
sleep 1

echo -e "${BLUE}[2/4] Starting godiam server1...${NC}"
...
echo -e "${BLUE}[4/4] Starting godiam client (sends 10 test messages)...${NC}"
