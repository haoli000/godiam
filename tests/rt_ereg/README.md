...
```
client ──► gw (rt_ereg: Destination-Realm="right\.side" → server1) ──► server1 (boosted)
                                                                           ──► server2
```
...
- **gw**: DRA relay with rt_ereg boosting server1 by score 200 when
  Destination-Realm matches "right\.side"
- **server1**: Backend peer boosted by regex match
- **server2**: Backend peer without boost

## Expected Behavior

All requests with Destination-Realm matching "right\.side" should be routed
