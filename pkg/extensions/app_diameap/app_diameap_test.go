...
type eapInitContext struct{ router *routing.Router }

func (c eapInitContext) GetDictionary() *dictionary.Dictionary   { return nil }
func (c eapInitContext) GetRouter() *routing.Router              { return c.router }
func (c eapInitContext) GetConfig() *config.Config               { return &config.Config{} }
func (c eapInitContext) GetPeers() []*peer.Peer                  { return nil }
func (c eapInitContext) GetStartTime() time.Time                 { return time.Now() }
func (c eapInitContext) GetExtensionManager() *extension.Manager { return nil }
func (c eapInitContext) GetEdgeRegistry() *edge.Registry         { return nil }

func TestNameAndStop(t *testing.T) {
	ext := &appDiamEAP{}
	if ext.Name() != "app_diameap" {
...
func TestInitRegistersEAPDispatchHandler(t *testing.T) {
	router := routing.NewRouter()
	ext := &appDiamEAP{}
	if err := ext.Init(eapInitContext{router: router}, nil); err != nil {
		t.Fatalf("Init failed: %v", err)
	}

	msg := message.NewAnswer(message.NewRequest(268, types.AppIDEAP))
	if err := router.Dispatch(nil, msg); err != nil {
		t.Fatalf("registered handler rejected an EAP answer: %v", err)
	}
}

func TestHandleEAPIgnoresAnswers(t *testing.T) {
	if err := handleEAP(nil, message.NewAnswer(message.NewRequest(268, types.AppIDEAP))); err != nil {
		t.Fatalf("answer handling returned error: %v", err)
	}
}

func TestHandleEAPReturnsSendErrors(t *testing.T) {
	...
	msg := message.NewRequest(268, types.AppIDEAP)
	if err := handleEAP(p, msg); err == nil {
		t.Fatal("closed peer send error was swallowed")
	}
}

func TestHandleEAPSendsSuccessAnswerWithPayload(t *testing.T) {
	peer.SetLocalConfig(peer.LocalConfig{DiameterIdentity: "edge.example.com", Realm: "example.com"})
	p, conn := openPeer(t)
	msg := message.NewRequest(268, types.AppIDEAP)
	msg.AddAVP(message.NewUTF8StringAVP(types.AVPCodeSessionID, types.AVPFlagMandatory, "eap-session"))
	msg.AddAVP(message.NewAVP(types.AVPCodeEAPPayload, types.AVPFlagMandatory, 0, []byte{1, 2, 3}))
	...
	if sessionID, ok := ans.GetSessionID(); !ok || sessionID != "eap-session" {
	...
	if payload := ans.FindAVP(types.AVPCodeEAPPayload, 0); payload == nil || !bytes.Equal(payload.Data, []byte{1, 2, 3}) {
		t.Fatalf("EAP-Payload = %+v, want request payload", payload)
	}
	if appID := ans.FindAVP(types.AVPCodeAuthApplicationID, 0); appID == nil {
		t.Fatal("Auth-Application-Id missing from EAP answer")
	} else if got, err := appID.GetUnsigned32(); err != nil || got != uint32(types.AppIDEAP) {
		t.Fatalf("Auth-Application-Id = %d (err=%v), want EAP", got, err)
	}
}
