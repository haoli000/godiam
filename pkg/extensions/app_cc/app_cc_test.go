...
type ccInitContext struct{ router *routing.Router }

func (c ccInitContext) GetDictionary() *dictionary.Dictionary   { return nil }
func (c ccInitContext) GetRouter() *routing.Router              { return c.router }
func (c ccInitContext) GetConfig() *config.Config               { return nil }
func (c ccInitContext) GetPeers() []*peer.Peer                  { return nil }
func (c ccInitContext) GetStartTime() time.Time                 { return time.Now() }
func (c ccInitContext) GetExtensionManager() *extension.Manager { return nil }
func (c ccInitContext) GetEdgeRegistry() *edge.Registry         { return nil }

func TestNameStopAndMetrics(t *testing.T) {
	ext := &appCC{}
	if ext.Name() != "app_cc" {
		t.Fatalf("Name() = %q, want app_cc", ext.Name())
	}
	if err := ext.Stop(); err != nil {
		t.Fatalf("Stop() returned error: %v", err)
	}
	metrics := ext.Metrics()
	if len(metrics) != 1 || metrics[0].Name != "requests_handled_total" || metrics[0].Type != extension.MetricCounter || metrics[0].Value != 0 {
		t.Fatalf("Metrics() = %+v, want zero-valued requests counter", metrics)
	}
}

func TestInitRegistersCreditControlApplication(t *testing.T) {
	r := routing.NewRouter()
	ext := &appCC{}
	if err := ext.Init(ccInitContext{router: r}, nil); err != nil {
		t.Fatalf("Init() returned error: %v", err)
	}

	p, remote := openCreditControlPeer(t)
	req := creditControlRequest(types.AppIDCreditControl)
	if err := r.Dispatch(p, req); err != nil {
		t.Fatalf("dispatching CCR through registered Credit-Control application: %v", err)
	}
	ans := readDiameterMessage(t, remote)

	assertBaseAnswer(t, ans, req, types.AppIDCreditControl)
	assertUnsigned32(t, ans.FindAVP(types.AVPCodeCCRequestType, 0), 1, "CC-Request-Type")
	assertUnsigned32(t, ans.FindAVP(types.AVPCodeCCRequestNumber, 0), 19, "CC-Request-Number")
	if got := ext.Metrics()[0].Value; got != 1 {
		t.Fatalf("requests metric = %v, want one handled CCR", got)
	}
}

func TestCreditControlIgnoresAnswers(t *testing.T) {
	ext := &appCC{}
	p, remote := openCreditControlPeer(t)
	req := creditControlRequest(types.AppIDCreditControl)
	if err := ext.handleCreditControl(p, message.NewAnswer(req)); err != nil {
		t.Fatalf("handling CCA: %v", err)
	}
	assertNoDiameterMessage(t, remote)
	if got := ext.Metrics()[0].Value; got != 0 {
		t.Fatalf("requests metric = %v, want answers ignored", got)
	}
}

func TestCreditControlDoesNotInventOptionalEchoes(t *testing.T) {
	ext := &appCC{}
	p, remote := openCreditControlPeer(t)
	...
	if err := ext.handleCreditControl(p, req); err != nil {
		t.Fatalf("handling minimal CCR: %v", err)
	}
	ans := readDiameterMessage(t, remote)
	...
	if ans.FindAVP(types.AVPCodeSessionID, 0) != nil || ans.FindAVP(types.AVPCodeCCRequestType, 0) != nil || ans.FindAVP(types.AVPCodeCCRequestNumber, 0) != nil {
		t.Fatal("CCA invented request AVPs that were absent from the CCR")
	}
}

func creditControlRequest(appID types.ApplicationID) *message.Message {
	msg := message.NewRequest(types.CmdCodeCreditControl, appID)
	msg.HopByHopID = 0x01020304
	msg.EndToEndID = 0x05060708
	msg.AddAVP(message.NewUTF8StringAVP(types.AVPCodeSessionID, types.AVPFlagMandatory, "cc-session;1"))
	msg.AddAVP(message.NewUnsigned32AVP(types.AVPCodeCCRequestType, types.AVPFlagMandatory, 1))
	msg.AddAVP(message.NewUnsigned32AVP(types.AVPCodeCCRequestNumber, types.AVPFlagMandatory, 19))
	return msg
}

func openCreditControlPeer(t *testing.T) (*peer.Peer, net.Conn) {
	t.Helper()
	return openAppPeer(t, []types.ApplicationID{types.AppIDCreditControl}, nil)
}
